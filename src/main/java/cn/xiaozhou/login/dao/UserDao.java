...
public interface UserDao {
    ...
    @Insert("insert into sys_user(username, password, nickname, headImgUrl, phone, telephone, email, birthday, sex, status, createTime, updateTime) " +
            "values(#{usernmae},#{password},#{nickname},#{headImgUrl},#{phone},#{telephone},#{email},#{birthday},#{sex},#{status},now(), now())")
    ...
    int saveUserRoles(@Param("userId") Long userId, @Param("roleId") List<Long> roleIds);
